...
const openingBadgerFrames=document.getElementById('openingBadgerFrames');const openingBadgerFrameSources=window.TEXUGUINHO_FRAMES||[];let openingBadgerTimer=null,openingBadgerFrameIndex=0;const openingBadgerPreloaded=[];function preloadOpeningBadgerFrames(){if(openingBadgerPreloaded.length)return;openingBadgerFrameSources.forEach(src=>{const im=new Image();im.src=src;openingBadgerPreloaded.push(im);});}function startOpeningBadger(){if(!openingBadgerFrames||!openingBadgerFrameSources.length)return;stopOpeningBadger();preloadOpeningBadgerFrames();openingBadgerFrameIndex=0;openingBadgerFrames.src=openingBadgerFrameSources[0];openingBadgerTimer=setInterval(()=>{openingBadgerFrameIndex=(openingBadgerFrameIndex+1)%openingBadgerFrameSources.length;openingBadgerFrames.src=openingBadgerFrameSources[openingBadgerFrameIndex];},66.6667);}function stopOpeningBadger(){if(openingBadgerTimer!==null){clearInterval(openingBadgerTimer);openingBadgerTimer=null;}}
